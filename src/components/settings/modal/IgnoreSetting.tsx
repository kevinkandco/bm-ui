...
const IgnoreSetting = ({
 ...
 const selectChannel = (channel: string) => {
  if (hasChannelActive && !slackData?.ignoreChannels?.includes(channel)) {
    setSlackData((prev) => ({
      ...prev,
...
const selectKeyword = (topic: string) => {
  if (hasKeywordActive && !slackData?.ignoreKeywords?.includes(topic)) {
    setSlackData((prev) => ({
      ...prev,
  ...
  const renderSelectedItems = () => {
    if (hasChannelActive && selectedTab === "channel" && slackData?.ignoreChannels?.length > 0) {
    ...
    } else if (hasKeywordActive && selectedTab === "keyword" && slackData?.ignoreKeywords?.length > 0) {
      return (
        <div className="flex flex-wrap gap-2 pt-3 mt-2">
