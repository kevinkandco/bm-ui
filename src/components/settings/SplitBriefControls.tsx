  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-text-secondary">
        Configure separate briefs for each tag with custom schedules and delivery preferences.
      </p>
        ...
        {tags.map((tag) => (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <Switch
                checked={tag.splitBriefEnabled}
                onCheckedChange={(enabled) => handleToggle(tag.id, enabled)}
              />
            </div>

            {tag.splitBriefEnabled && (
              <div className="space-y-4 pl-8 border-l-2 border-white/10">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <input
                    type="time"
                    value={tag.splitBriefTime}
                    onChange={(e) => handleTimeChange(tag.id, e.target.value)}
                    className="px-3 py-1 bg-white/5 border border-white/20 rounded text-text-primary text-sm focus:ring-2 focus:ring-accent-primary focus:border-transparent"
                  />
                </div>
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          id={`${tag.id}-email`}
                          checked={tag.splitBriefEmail}
                          onCheckedChange={(checked) => 
                            handleDeliveryChange(tag.id, 'email', checked as boolean)
                          }
                        />
                        <label 
                          htmlFor={`${tag.id}-email`}
                      ...
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          id={`${tag.id}-audio`}
                          checked={tag.splitBriefAudio}
                          onCheckedChange={(checked) => 
                            handleDeliveryChange(tag.id, 'audio', checked as boolean)
                          }
                        />
                        <label 
                          htmlFor={`${tag.id}-audio`}
                          className="text-sm text-text-primary cursor-pointer"
                        >
                          Audio
                        </label>
                ...
                <div className="mt-3 p-2 bg-white/5 rounded border border-white/5">
                  <p className="text-xs text-text-secondary">
                    ...
                    {[tag.splitBriefEmail && 'email', tag.splitBriefAudio && 'audio']
                      .filter(Boolean)
                      .join(' and ') || 'no delivery method selected'}
                  </p>
                </div>
              </div>
            )}
      ...
      {tags.filter(tag => tag.splitBriefEnabled).length === 0 && (
