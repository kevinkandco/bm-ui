...
const HomeView = ({
  ...
  const [selectedBrief, setSelectedBrief] = useState<number | null>(1);
  ...
  const handlePlayBrief = useCallback((briefId: number) => {
    if (playingBrief === briefId) {
      ...
      setSelectedTranscript({
        ...
        stats: { interrupts: 14, focusTime: "2h 17m", timeSaved: "~66" }
      });
    ...
    return (
      <Popover>
        <PopoverTrigger asChild>
          <Button variant="ghost" className={cn("bg-transparent text-xs px-2 py-1 rounded-full font-medium border h-auto hover:bg-surface-raised/20", 
          ...
          <div className="space-y-1">
            {["High", "Medium", "Low"].map(priority => (
              <Button 
                key={priority} 
                variant="ghost" 
                className={cn("w-full justify-start text-xs rounded-full", 
  ...
  const getStatusChip = () => {
    const getStatusIcon = () => {
      switch (userStatus) {
        case 'focus': return <Focus className="h-3 w-3" />;
        case 'away': return <Clock className="h-3 w-3" />;
        case 'vacation': return <X className="h-3 w-3" />;
      ...
      <div className="flex-1 pb-20 flex">
              ...
              <div className="p-4">
                ...
                <div className="mt-4">
                  <Tabs value={leftRailTab} onValueChange={(value) => setLeftRailTab(value as 'briefs' | 'calendar' | 'followups')} className="w-full">
                    ...
                    <TabsContent value="briefs" className="mt-4 flex-1 min-h-0">
                      ...
                      <div className="space-y-4">
                        <div className="bg-surface-raised/30 rounded-lg border border-border-subtle">
                          <Table>
                            ...
                            <TableBody>
                              {followUps.map(item => (
                                <TableRow key={item.id} className="border-border-subtle hover:bg-surface-raised/20 cursor-pointer">
        ...
        <div className="flex-1 h-screen overflow-hidden">
          <div className="h-full bg-background/80 backdrop-blur-sm shadow-xl rounded-xl border border-border-subtle overflow-hidden">
            <p className="p-8 text-text-secondary">Main content area...</p>
          </div>
        </div>
