...
import { Clock, Zap, X, Calendar, SkipForward, Calendar as CalendarIcon } from "lucide-react";
...
interface CatchMeUpWithSchedulingProps {
  open: boolean;
  onClose: () => void;
  onGenerateSummary: (timeDescription: string, skipScheduled?: boolean) => void;
  upcomingBriefName?: string;
  upcomingBriefTime?: string;
}

const CatchMeUpWithScheduling = ({ 
  open, 
  onClose, 
  onGenerateSummary, 
  upcomingBriefName, 
  upcomingBriefTime 
}: CatchMeUpWithSchedulingProps) => {
  ...
  const [scheduledBriefAction, setScheduledBriefAction] = useState<"keep" | "skip">("keep");
  ...
  const handleGenerate = () => {
    ...
    const skipScheduled = scheduledBriefAction === "skip";
    onGenerateSummary(timeDescription, skipScheduled);
  };
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className={`sm:max-w-md max-h-[90vh] overflow-y-auto bg-background/80 backdrop-blur-xl border border-white/10 ${isMobile ? 'p-4' : 'p-6'}`}>
        ...
        <div className="space-y-4 py-2">
          ...
          {/* Scheduled Brief Options */}
          {upcomingBriefName && upcomingBriefTime && (
            <div className="space-y-3 pt-2 bg-white/10 rounded-lg border border-white/10 p-3">
              <div className="flex items-center gap-2 text-white">
                <CalendarIcon className="h-4 w-4 text-blue-400" />
                <span className="text-sm font-medium">Upcoming: {upcomingBriefName} at {upcomingBriefTime}</span>
              </div>
              
              <RadioGroup 
                value={scheduledBriefAction}
                onValueChange={(value) => setScheduledBriefAction(value as "keep" | "skip")}
                className="space-y-2"
              >
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="keep" id="keep" />
                  <Label htmlFor="keep" className="text-white text-sm">Keep scheduled brief</Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="skip" id="skip" />
                  <Label htmlFor="skip" className="text-white text-sm">Skip scheduled brief</Label>
                </div>
              </RadioGroup>
            </div>
          )}
...
export default CatchMeUpWithScheduling;
