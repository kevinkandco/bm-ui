...
const EndFocusModal = ({
  ...
  return <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className={`sm:max-w-md bg-background/80 backdrop-blur-xl border border-white/10 ${isMobile ? 'p-4' : 'p-6'}`}>
        ...
        <div className="py-4">
          <ListeningScreen isListening={true} title="Brief Me is listening" subtitle="Analyzing your updates and creating your brief..." />
          ...
          <div className="flex flex-col items-center space-y-4 mt-6">
            ...
            <div className="w-full max-w-xs">
              <div className="h-2 bg-white/10 rounded-full w-full overflow-hidden">
                <div className="h-full bg-primary-teal rounded-full transition-all duration-1000 ease-linear" style={{
                width: `${progress}%`
              }} />
              </div>
            </div>
            
            <Button onClick={onClose} variant="outline" size="sm" className="bg-white/10 border-white/20 text-white hover:bg-white/20 rounded-full mt-4">
              Stop
            </Button>
          </div>
        </div>
