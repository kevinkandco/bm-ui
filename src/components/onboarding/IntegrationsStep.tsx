  ...
  return (
    <div className="space-y-4 sm:space-y-6">
        ...
        <div
          ...
          style={{
            ...
            borderBottom: isMobile
              ? "24px solid rgba(0, 224, 213, 0.3)"
              : "32px solid rgba(0, 224, 213, 0.3)",
          }}
        />
      </div>

      <div className="space-y-1 sm:space-y-2">
        <h2 className="text-xl sm:text-2xl font-semibold text-text-primary tracking-tighter">
          Connect your tools
        </h2>
        <p className="text-sm sm:text-base text-text-secondary">
          Brief.me will monitor these sources to create your personalized brief.
        </p>
      </div>
      ...
      <div className="p-2 sm:p-3 bg-white/10 backdrop-blur-md rounded-lg border border-black/20 dark:border-white/20">
        <h4 className="text-[10px] sm:text-xs font-medium text-text-primary mb-1 sm:mb-2">
          More integrations coming soon:
        </h4>
        <div className="flex flex-wrap gap-1 sm:gap-1.5">
          {[
            ...(groupedIntegrations.V3 || []),
            ...(groupedIntegrations.Future || []),
          ].map((integration) => (
            <span
              key={integration.id}
              className="text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 bg-white/15 rounded-full text-text-primary/80 font-medium dark:text-white/80"
            >
              {integration.name}
            </span>
          ))}
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                variant="ghost"
                size="sm"
                className="p-0 h-auto text-text-secondary"
              >
      ...
      <div className="flex justify-between pt-2">
        ...
        <Button
          ...
          disabled={!hasAnyConnection || loader}
          variant="primary"
          size="pill"
          className="disabled:opacity-50 disabled:pointer-events-none py-2 sm:py-3 px-3 sm:px-4 text-sm"
        >
          {loader && (
            <svg
              aria-hidden="true"
      ...
      <div className="flex justify-center">
        <Button
          variant="link"
          onClick={onSkip}
          className="text-xs sm:text-sm text-text-secondary hover:text-text-primary font-medium"
        >
          Skip for now
        </Button>
      </div>
