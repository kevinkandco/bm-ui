...
const OnboardingLayout = ({ children, className }: OnboardingLayoutProps) => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center px-4 pb-6 pt-0 relative overflow-hidden bg-surface">
      ...
      <div 
        className={`absolute left-1/4 top-10 ${isMobile ? 'w-16 h-16' : 'w-24 h-24'} rounded-xl glass-reflection z-[2] opacity-20`}
        style={{
      ...
      ></div>
      <div 
        className={`absolute right-1/4 bottom-1/5 ${isMobile ? 'w-12 h-12' : 'w-16 h-16'} rounded-xl glass-reflection animate-float-delay z-[2]`}
        style={{
      ...
      <div 
        className={cn(
          "w-full max-w-md z-10 p-6 sm:p-8 mt-0 backdrop-blur-xl rounded-3xl border border-border-subtle glass-reflection",
          className
        )}
        style={{
      ...
      <div className="mt-3 text-xs text-text-secondary z-10 px-4 text-center">
