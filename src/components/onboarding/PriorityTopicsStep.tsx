...
const PriorityTopicsStep = ({
  onNext,
  onBack,
  updateUserData,
  userData
}: PriorityTopicsStepProps) => {
  ...
  return <div className="space-y-5 sm:space-y-8">
      ...
      <div className="space-y-4 sm:space-y-6">
        <div className="space-y-2 sm:space-y-3">
          ...
          <div className="flex gap-2">
            <Input id="priority-topic" placeholder="Enter keyword or topic" value={inputValue} onChange={e => setInputValue(e.target.value)} onKeyPress={e => e.key === 'Enter' && addTopic()} className="bg-white/15 border-white/20 text-off-white placeholder:text-white/50 text-sm" />
            
          </div>
          
          {priorityTopics.length > 0 && <div className="flex flex-wrap gap-1 sm:gap-2 pt-2">
              {priorityTopics.map(topic => <div key={topic} className="flex items-center gap-1 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full bg-glass-blue/10 border border-glass-blue/40 text-xs sm:text-sm text-off-white">
        ...
        <div className="space-y-2 sm:space-y-3 pt-1 sm:pt-2">
          ...
          <div className="flex flex-wrap gap-1 sm:gap-2">
            {suggestedTopics.map(topic => <button key={topic.id} onClick={() => addSuggestedTopic(topic)} className={cn("px-2 sm:px-3 py-1 sm:py-1.5 rounded-full border text-xs sm:text-sm", priorityTopics.includes(topic.name) ? "bg-glass-blue/10 border-glass-blue/40 text-glass-blue" : "bg-white/10 border-white/20 text-off-white hover:border-white/40")}>
          ...
          <div className={`${isMobile ? 'space-y-4' : 'grid grid-cols-1 sm:grid-cols-2 gap-4'} mt-4`}>
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-1 sm:gap-2">
                {suggestedTopics.filter(t => t.category === "Priority" || t.category === "Work").map(topic => <button key={`cat-${topic.id}`} onClick={() => addSuggestedTopic(topic)} className={cn("px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs", priorityTopics.includes(topic.name) ? "bg-glass-blue/10 border border-glass-blue/40 text-glass-blue" : "bg-white/10 text-off-white/70 hover:text-off-white")}>
                      {topic.name}
                    </button>)}
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-1 sm:gap-2">
                {suggestedTopics.filter(t => t.category === "Technical").map(topic => <button key={`cat-${topic.id}`} onClick={() => addSuggestedTopic(topic)} className={cn("px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs", priorityTopics.includes(topic.name) ? "bg-glass-blue/10 border border-glass-blue/40 text-glass-blue" : "bg-white/10 text-off-white/70 hover:text-off-white")}>
                      {topic.name}
                    </button>)}
              </div>
            </div>
      ...
      <div className="flex justify-between pt-2 sm:pt-4">
        ...
        <Button onClick={handleContinue} variant="primary" size="pill" className="py-2 sm:py-3 px-4 sm:px-6 text-sm">
          Continue
        </Button>
      </div>
    </div>;
